...
namespace FaceDetection.Library.Algorithm
{
    class FaceRecognitionAlgorithm
    {
            ...
            using (var originalImage = new Image<Bgr, byte>(faceImage))
            {
                try
                {
                    ...
                    if (faces.Count == 1)
                    {
                        ...
                        //using (System.Drawing.Image MySystemImage = originalImage.ToBitmap())
                        //{

                        //    MySystemImage.Save(localOrignalImagePath, ImageFormat.Jpeg);
                        //    //MySystemImage.Save(localOrignalImagePath);
                        //}
                        //thumbnailGrayscaleIplImage.Save(localThumbnailGrayscaleImagePath);
                        FileHelper.Save(originalImage, localOrignalImagePath, 90);
                        FileHelper.Save(thumbnailGrayscaleIplImage, localThumbnailGrayscaleImagePath, 90);
        ...
        private Image<Gray, byte> CropAndNormalizeFace(Image<Bgr, byte> inputImage, Face face)
        {
            ...
            {
                return thumbnailGrayFace.Resize(_faceSize.Width, _faceSize.Height,Emgu.CV.CvEnum.Inter.Cubic);
            }
        }
